...
function head() {
	...
	echo	'<head>';
	echo		'<title>Dement.ru :: Всплывающее окно</title>';
	...
	echo		'<script type="text/javascript">';					// скрипт :: всплывающее окно
	...
	echo			'"MovingUp",';								// 							эффект появления объекта (MoveUp, MoveDown ... ) 
	echo			'"300px",';										// width dialog				ширина окна
	echo			'"400px",';										// height dialog			высота окна	
	...
	echo			'"'.$inner.'",';								// text content				содержимое всплывающего окна
	...
	echo			'"TRUE",';										// показывать ли задний фон
	...
	// пример совместного использования двух всплывающих окон		

	echo		'<script type="text/javascript">';					// срипт :: всплывающее окно
	echo			'new deShowDialog(';			
	echo			'"goSecond",';									// object 					объект с которым связано событие появления всплывающего окна
	echo			'"click",';										// startEvent				событие при котором появляется всплывающее окно 
	echo			'"MovingLeft",';								// 							эффект появления объекта (MoveUp, MoveDown ... ) 
	echo			'"300px",';										// width dialog				ширина окна
	echo			'"400px",';										// height dialog			высота окна	
	echo			'"#999999 solid 10px",';						// border dialog			границы окна CSS 
	echo			'"200",';										// z-index dialog			порядок данного слоя	
	echo			'"90",';										// opacity % dialog		    непрозрачность этого окна  
	echo			'"200",';										// (мс) effect  Speed dialog     скорость появления окна
	echo			'"deModalDialog2",';							// dialog_id				id главного окна 	

	echo			'"<strong>Второе окно</strong>",';				// caption topLine			заголовок окна 
	echo			'"Verdana, Arial, Helvetica, sans-serif",';		// font-family topLine		тип шрифта заголовка 
	echo			'"12px",';										// font-size topLine		размер шрифта заголовка	
	echo			'"#333333",';									// font-color topLne		цвет шрифта заголовка 
	echo			'"30px",';										// height topLne			высота заголовка	
	echo			'"#dddddd",';									// background topLne		задний фон заголовка 
	echo			'"center",';									// text-align topLine		выравнивание данного заголовка
	echo			'"deUpperLine2",';								// topLne_id				id полосы с заголовком 	

	echo			'"'.$inner2.'",';										// text content				содержимое всплывающего окна
	echo			'"Verdana, Arial, Helvetica, sans-serif",';		// font-family content		тип шрифта этого содержимого
	echo			'"12px",';										// font-size content		размер шрифта этого содержимого
	echo			'"#333333",';									// font-color content		цвет шрифта 
	echo			'"#ffffff",';									// background content		задний фон 
	echo			'"left",';										// text-align content		выравниевание текста
	echo			'"deMainContent2",';							// content_id				id оболочки с основным содержимым 

	echo			'"TRUE",';										// показывать ли задний фон
	echo			'"#000000",';									// background backWall		цвет фона которым закрашивается все позади окна. если он вам не нужен то поставьте none 
	echo			'"70",';										// opacity % backWall		непрозрачность этого фона.  
	echo			'"deBackWall_2",';								// backwall_id				id дива с этим фоном 	

	echo			'"TRUE",';										// отображать или нет кнопку выхода Х 
	echo			'"url(http://www.dement.ru/!lib/showModalDialog/img/btnClose.gif) no-repeat center",';	// фон данной кнопки
	echo			'"ClassButtonClose2",';							// ВНИМАНИЕ!!! класс для <span> внутри которого будут ссылки при нажатии на которые будет закрываться окно !!! 
...
function action() { 
	...
	echo		'Это тестовая страница для проверки взаимодействия двух всплывающих окон (модуль deShowDialog).<br>';
	echo		'Также к этим модулям добавлен эффект drag&drop (модуль deDragDrop).<br>';
	echo		'<br>';
	echo		'<a id="testLink" href="#de">Нажмите сюда чтобы вызвать первое модальное окно</a>';
	echo	'</div>';
