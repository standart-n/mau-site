...
function showPages() {
	...
	$show.=		"<td align=\"left\">";
	if ($this->list_now>1) {
		$show.=			"<div class=\"clLinkList\"><a onClick=\"".$this->type."_list_prev();\" href=\"#de\">Предыдущая</a></div>";
	} else {
	...
	if ($this->list_now<$this->list_count) {
		$show.=			"<div class=\"clLinkList\"><a onClick=\"".$this->type."_list_next();\" href=\"#de\">Следующая</a></div>";
	} else {
...
function showLimit() {
	...
	$show.=	"<tr>";
	$show.=		"<td align=\"left\">";
	$show.=			"<span class=\"clTextList\">Выводить по </div>";
	if ($this->row_max!=5) {
	...
	$show.=			"<span class=\"clTextList\"> позиций на странице </div>";
	$show.=		"</td>";
	$show.=	"</tr>";
	$show.="</table>";
	$show.=	"</div>";
