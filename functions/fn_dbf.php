...
class fn_dbf {
    ...
    function getRow($recnum) {
        ...
        for ($i=0; $i<$this->dbf_num_field; $i++) {
            $col=trim(strval(htmlspecialchars(substr($rawrow,$beg,$this->dbf_names[$i]['len']))));
            //$first_sym=substr($col,0,1);
            /*if ((intval($first_sym)>0) && ($i==5)) {
                echo $col."<br>";
                $col=trim(substr($rawrow,$beg+1,$this->dbf_names[$i]['len']));
                echo $col."<br>";
            }*/
//            $col=iconv('CP866','UTF-8',$col);
            //$col=str_replace("¦","",$col);
            //$col=str_replace("¦","",$col);
            
            $col=iconv('CP866','CP1251',$col);
